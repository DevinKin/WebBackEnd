...
import cn.devinkin.jk.domain.${ className };
import cn.devinkin.jk.utils.Page;
import cn.devinkin.jk.service.${ className }Service;
...
public class ${ className }Action extends BaseAction implements ModelDriven<${ className }> {
	...
	public String submit() throws Exception {
		...
		${ className?uncap_first }Service.changeState(ids, 1);
	...
	public String cancel() throws Exception {
		...
		String[] ids = model.getId().split(", ");
		// 2. 遍历ids，并加载出每个装箱单的对象，再修改装箱单的状态
		${ className?uncap_first }Service.changeState(ids, 0);
